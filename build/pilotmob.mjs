...
await c.send('Emulation.setDeviceMetricsOverride',{width:390,height:860,deviceScaleFactor:1,mobile:true});
await c.send('Page.navigate',{url:URL});await sleep(2600);
await c.send('Runtime.evaluate',{expression:"document.querySelector('.a2-core').scrollIntoView({block:'center'})"});
await sleep(1200);
let r=await c.send('Page.captureScreenshot',{format:'png'});
writeFileSync(`${DIR}/pilot-mobile.png`,Buffer.from(r.result.data,'base64'));
console.log('mobile shot done');process.exit(0);
