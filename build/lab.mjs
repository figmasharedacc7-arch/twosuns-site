...
for(const [sel,out] of [['.hub','lab-hub.png'],['.spine','lab-spine.png']]){
  await c.send('Runtime.evaluate',{expression:`document.querySelector('${sel}').scrollIntoView({block:'center'})`});
  await sleep(900);
  const r=await c.send('Page.captureScreenshot',{format:'png'});
  writeFileSync(`${DIR}/${out}`,Buffer.from(r.result.data,'base64'));console.log('shot',out);
}
const ov=await c.send('Runtime.evaluate',{expression:"JSON.stringify({doc:document.documentElement.scrollWidth,vw:innerWidth})",returnByValue:true});
console.log('overflow check', ov.result.result.value);
process.exit(0);
